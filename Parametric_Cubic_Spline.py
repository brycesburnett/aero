...
eps = []
BB = []
AA = []	#declaring as 1d but can assign 2D properties later
dydxo = 0
dydxL = 0
dydxR = 0
...
def single_curve_airfoil():
	...
	#dzdt_S = []
	#d2zdt2_S = []
	print "TAU_S: ", len(tau_S)
	print "zet_S: ", len(zet_S)
...
def tau_chi(tau, deLta, chi, chi0, chid, chidd):
	chi0 = 1 - math.sin(PI_RAD * tau) 
	
	chi = 1 - ((1 - deLta) * math.sin(math.pi * tau) + 
	deLta * math.sin(3 * PI_RAD * tau))
	
	chid = -((1 - deLta) * PI_RAD * math.cos(PI_RAD * tau) + 
	3 * PI_RAD * deLta * math.cos(3 * PI_RAD * tau))
	...
	for i in range (i, ns):
		...
		for i in range (i, ns):
			if (Xa <= xx[i] and Xb >= xx[i + 1]):
				area = (area +
				yy[i] * _del[i] + dydx[i] * ((_del[i]**2)/2) +
				zz[i] * ((_del[i]**3)/6) +
				(zz[i+1] - zz[i]) * ((_del[i]**3)/24))
			...
			if (Xa > xx[i] and Xa < xx[i + 1]):
				...
				zz[i] * (((Xa - xx[i])**3)/6) -
				(zz[i + 1] - zz[i]) * ((Xa - xx[i])**4)/(24 * _del[i]))
...
def CS_intrp(nn, xx, yy, zz, dydx, Xo, Yo, dydxo, d2ydx2o):
	...
	ns = nn - 1 
	#ns = number of splines for nn points
	...
	#linear extrapolate right
		...
		for i in range (1, ns):
			if(xx[i + 1] >= Xo):
				xmxi = (Xo - xx[i])
				epsi = eps[i]; deli = _del[i];
				yyi = yy[i]; zzi = zz[i]
				dydxi = dydx[i]; zip1 = zz[i + 1]
